...
class FixedKLController:
    ...
    def __init__(self, kl_coef, multiturn_kl=False, kl_coef_turn1=0.1, kl_coef_turn2=0.001):
        self.value = kl_coef
        self.multiturn_kl = multiturn_kl
        self.kl_coef_turn1 = kl_coef_turn1
        self.kl_coef_turn2 = kl_coef_turn2
...
def get_kl_controller(kl_ctrl):
    if kl_ctrl.type == 'fixed':
        return FixedKLController(kl_coef=kl_ctrl.kl_coef, multiturn_kl=kl_ctrl.multiturn_kl, kl_coef_turn1=kl_ctrl.kl_coef_turn1, kl_coef_turn2=kl_ctrl.kl_coef_turn2)
...
def compute_gae_advantage_return(token_level_rewards: torch.Tensor, values: torch.Tensor, eos_mask: torch.Tensor, gamma: torch.Tensor, lam: torch.Tensor, multiturn_mask: torch.Tensor = None, norm_type="whole"):
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(gen_len)):
            nextvalues = values[:, t + 1] * multiturn_mask[:, t + 1] if t < gen_len - 1 else 0.0
            lastgaelam = lastgaelam * multiturn_mask[:, t + 1] if t < gen_len - 1 else 0.0
        ...
        if norm_type == "whole":
            advantages = verl_F.masked_whiten(advantages, multiturn_mask)
        elif norm_type == "role":
            advantages = verl_F.masked_whiten_task(advantages, multiturn_mask)
...
def compute_reinforce_plus_plus_outcome_advantage(token_level_rewards: torch.Tensor, response_mask: torch.Tensor,
                                                  gamma: torch.Tensor, multiturn_mask: torch.Tensor = None, norm_type="whole"):
    """
    ...
    with torch.no_grad():
        ...
        for t in reversed(range(gen_len)):
            ...
            running_return = running_return * multiturn_mask[:, t]
            returns[:, t] = running_return


        if norm_type == "whole":
            advantages = verl_F.masked_whiten(returns, multiturn_mask)
        elif norm_type == "role":
            advantages = verl_F.masked_whiten_task(returns, multiturn_mask)
        advantages = advantages * multiturn_mask
    return advantages, returns
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
        ...
        max_token = torch.sum(torch.ones_like(loss_mat))
        seq_losses = torch.sum(loss_mat * loss_mask) / max_token  # token-mean
        loss = torch.mean(seq_losses)  # seq-mean
    else:
