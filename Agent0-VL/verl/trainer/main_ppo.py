...
class TaskRunner:
    ...
    # elif reward_manager_name == 'pag':
    #     from verl.workers.reward_manager import PAGRewardManager
    #     reward_manager_cls = PAGRewardManager
    # else:
    #     raise NotImplementedError
    
    # # assert reward_manager_cls == PAGRewardManager, "Only multi-turn reward manager is supported for now"
    ...
    # val_reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=1, config=config)
        ...
        if reward_manager_name == 'multiturn':
            ...
            from verl.workers.reward_manager import PAGRewardManager
            reward_manager_cls = PAGRewardManager
            val_reward_manager_cls = PAGRewardManager
        elif reward_manager_name == 'standalone_genrm':
            from verl.workers.reward_manager import StandaloneGenRMRewardManager
            from verl.workers.reward_manager import PAGRewardManager
            reward_manager_cls = StandaloneGenRMRewardManager
            val_reward_manager_cls = PAGRewardManager
        elif reward_manager_name == 'agent0':
            ...
            val_reward_manager_cls = EvoTIRRewardManager
        else:
            raise NotImplementedError(f"Unknown reward_manager: {reward_manager_name}")
        # compute_score = get_custom_reward_fn(config)
        reward_fn = reward_manager_cls(tokenizer=tokenizer, num_examine=0, config=config.reward_model)

        # Note that we always use function-based RM for validation
        val_reward_fn = val_reward_manager_cls(tokenizer=tokenizer, num_examine=1, config=config.reward_model)
